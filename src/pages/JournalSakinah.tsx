...
const WRITING_PROMPTS = [
  ...
  "Quel est ton objectif le plus important cette semaine ?",
  "Qu'est-ce qui te rapproche d'Allah aujourd'hui ?",
  "Quel acte de bonté as-tu fait ou reçu ?",
  "Comment s'est passée ta journée en un mot ?",
];
...
const JournalSakinah = () => {
  ...
  const loadEntries = async () => {
    if (!user) return;
    // Load own entries + shared entries from partner
  ...
  const saveEntry = async () => {
    ...
    if (error) {
      ...
      if (mood <= 2 && profile) {
        ...
        toast.info(`${partnerName}, ${profile.display_name} a besoin d'un mot doux maintenant 💛`, { duration: 6000 });
      }
      setContent("");
      setMood(3);
      setShowEditor(false);
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        >
          <h1 className="text-2xl font-display font-bold text-foreground">🌙 Journal Sakinah</h1>
          <p className="text-xs text-muted-foreground">Espace de sérénité et d'introspection</p>
        </motion.div>
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="glass rounded-xl p-5 text-center space-y-3 glow-border-gold"
        >
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                <motion.button
                  ...
                  className="px-4 py-2.5 rounded-lg bg-secondary border border-border text-foreground text-sm"
                >
                  Annuler
                </motion.button>
              </div>
        ...
        <div className="space-y-3">
          ...
          {entries.map(entry => (
            ...
            >
              ...
              {entry.prompt_used && (
                <p className="text-[10px] italic text-accent/60">{entry.prompt_used}</p>
              )}
          ...
          {entries.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-xs mt-1">Commencez à écrire vos pensées...</p>
            </div>
          )}
        </div>
      </div>
