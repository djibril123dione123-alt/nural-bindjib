...
interface Mission {
  id: string; from_user_id: string; to_user_id: string; title: string; description: string;
  xp: number; status: string; proof_url: string | null; created_at: string; completed_at: string | null;
}
...
export default function AlterEgoLabContent() {
  ...
  useEffect(() => {
    if (!user) return;
    loadPartner(); loadMissions();
    const channel = supabase.channel("missions-rt").on("postgres_changes", { event: "*", schema: "public", table: "alter_ego_missions" }, () => loadMissions()).subscribe();
  ...
  const loadMissions = async () => {
    if (!user) return;
    const { data } = await supabase.from("alter_ego_missions").select("*").or(`from_user_id.eq.${user.id},to_user_id.eq.${user.id}`).order("created_at", { ascending: false });
    if (data) setMissions(data as Mission[]);
  };

  const createMission = async () => {
    if (!user || !partnerId || !title.trim()) return;
    await supabase.from("alter_ego_missions").insert({ from_user_id: user.id, to_user_id: partnerId, title: title.trim(), description: desc.trim(), xp });
    setTitle(""); setDesc(""); setXp(20);
  ...
  return (
    <div className="space-y-5">
      <div className="glass rounded-xl p-5 space-y-3">
        <h3 className="text-sm font-semibold text-primary">Mission pour {partnerName}</h3>
        <Input placeholder="Titre..." value={title} onChange={e => setTitle(e.target.value)} className="bg-secondary/50 border-border" />
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button whileTap={{ scale: 0.95 }} onClick={createMission} disabled={!title.trim() || !partnerId}
            className="ml-auto px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold disabled:opacity-40">Envoyer ⚔️</motion.button>
        </div>
      </div>
          ...
          <motion.div key={m.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }}
            ...
            {m.status === "pending" && tab === "received" && (
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => completeMission(m)}
                className="w-full py-2 rounded-lg bg-primary/20 text-primary text-sm font-semibold">✅ Fait</motion.button>
            )}
            {m.status === "completed" && <p className="text-xs text-primary">✅ Accomplie</p>}
          </motion.div>
        ))}
      </AnimatePresence>

      {displayed.length === 0 && <p className="text-center text-sm text-muted-foreground py-8">Aucune mission pour l'instant.</p>}
    </div>
  );
}
