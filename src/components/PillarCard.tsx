...
interface PillarCardProps {
  ...
  progress: number;
  onToggle: (id: string, pillar: string, xpValue?: number) => void;
  customQuests?: Quest[];
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className={`font-display text-lg font-semibold tracking-wide ${accentClass}`}>{pillar.name}</h3>
        </div>
        <div className="flex items-center gap-2">
        ...
        {showAdd && (
          ...
          >
            ...
            <Input
              ...
              max={100}
            />
            <motion.button whileTap={{ scale: 0.9 }} onClick={handleAdd} className="h-8 px-3 rounded-md bg-primary text-primary-foreground text-xs font-semibold">
              OK
            </motion.button>
          </motion.div>
        )}
            ...
            return (
              <motion.div
                ...
                className={`w-full flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors ${
                  isDone ? "bg-primary/10 border border-primary/20" : "bg-secondary/50 hover:bg-secondary"
                }`}
              >
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  animate={isDone ? { scale: [1, 1.05, 0.98, 1] } : { scale: 1 }}
                  transition={{ duration: 0.24, ease: "easeOut" }}
                  onClick={() => onToggle(quest.id, pillar.id, quest.xp)}
                ...
                >
                  ...
                  <span className={`text-sm flex-1 ${isDone ? "line-through text-muted-foreground" : "text-foreground"}`}>{quest.label}</span>
                </motion.button>
                {quest.optional && (
                ...
                {isCustom && onDeleteCustom && (
                  <motion.button whileTap={{ scale: 0.9 }} onClick={() => onDeleteCustom(quest.id)} className="text-destructive/60 hover:text-destructive transition-colors">
